...
class VideoFrameGenerator(Sequence):
    ...
    def _get_frames(self, video, nbframe, shape, force_no_headers=False):
        ...
        fps = 3  # round(cap.get(cv.CAP_PROP_FPS))

        frame_nums = []
        # for i in range(int(total_frames)):
        #     if i % frame_step == 0:
        #         frame_nums.append(i)
        # frames_num = random.randint(30, total_frames)
        frame_nums.sort()
        # print('frames: ',frame_nums)
        frame_offset = random.randint(0, fps)
        ...
        while True:
            ...
            if (frame_i - frame_offset) % (fps) == 0:
                # if frame_i % frame_step == 0:
                # if frame_i in frame_nums:
                # resize
        ...
        if self.nb_channel == 3:
            blank_image = np.zeros(shape=(shape[0], shape[1], 3))  # , np.uint8)
        else:
            blank_image = np.zeros(shape=shape)

        len_frames = len(frames)
        if len_frames < nbframe:
            for i in range(len_frames, nbframe):
                frame = img_to_array(blank_image) * self.rescale
                frames.append(frame)
        elif len_frames > nbframe:
            randIndex = random.sample(range(len_frames), nbframe)
            randIndex.sort()
            tmp = [frames[i] for i in randIndex]
            frames = tmp

        assert len(frames) == nbframe
